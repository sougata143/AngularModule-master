...
import { countryCreateModel } from '../../../models/country/countryCreate.model';
...
export class createCountryComponent implements OnInit {
	...
	public countryCreateModel: countryCreateModel = new countryCreateModel();
	public createcountryresponsedata: any;
	...
	public columnDef : any = [
			...
			{headerName: "COUNTRY NAME", field: "countryName", minWidth : 250, editable : true, cellEditor : 'text'},
	...
	constructor(public http: HttpTestService, public router: Router, public session: sessionServices) { 
			...
			this.gridOptions.onCellEditingStopped = function(params) {
				if(self.countryList != 'undefined'){
				   self.updateDeviation(params.node.id);
				   params.api.setRowData(self.countryList);
				}
				};
	...
	createCountry = function(){
			var self = this;
			this.http.createCountry(self.countryCreateModel)
				.subscribe(
				(data) => {
					self.createcountryresponsedata = data;
					self.loadcountryList()
				},
				(error) => self.errorMsg = error,
				() => {
					self.successMsg = "Created Successfully.",
					self.countryCreateModel.countryName= ""
					
				}
			);
			var newItem =self.countryCreateModel;
...
updateDeviation = function(index){
			var self = this;
			self.countryList[index].deviation = (self.countryList[index].countryName - self.countryList[index].countryName);
			self.http.editCountry( self.countryList[index] )
			.subscribe(
				(data) => {
			...
			var newItem =self.createcountryData;
			this.gridOptions.api.updateRowData({add: [newItem]});
}
//delete data
...
deleteItem = function(indexdId){
			var self = this;
			this.gridOptions.api.setRowData(self.countryList);
			var id = self.countryList[indexdId].id
				this.http.deleteCountry(id)
				.subscribe(
				() => {

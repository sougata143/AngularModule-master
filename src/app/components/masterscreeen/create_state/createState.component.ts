...
import { stateCreateModel } from '../../../models/state/stateCreate.model';

@Component({
  selector: 'app-payroll',
  templateUrl: './createState.component.html'
})
export class createStateComponent implements OnInit {
	...
	public stateCreateModel: stateCreateModel = new stateCreateModel();
	public gridOptions: GridOptions;
	public createstateresponsedata: any;
	...
	public allocationColumnDef : any = [
			{headerName: "S NO.", field: "id", minWidth : 150},
			{headerName: "COUNTRY NAME", field: "country.countryName", minWidth : 250},
			{headerName: "STATE NAME", field: "stateName", minWidth : 250, editable : true, cellEditor : 'text'},
	...
	constructor(public http: HttpTestService, public router: Router, public session: sessionServices) { 
			...
			this.gridOptions.columnDefs = this.allocationColumnDef;
			...
			this.gridOptions.onCellEditingStopped = function(params) {
				if(self.stateList != 'undefined'){
				   ...
				   params.api.setRowData(self.stateList);
				}
				};
...
//get session details
	...
	createState = function(){
			var self = this;
			this.http.createState(self.stateCreateModel)
				.subscribe(
				(data) => {
					self.createstateresponsedata = data;
					self.loadstateList()
				},
				(error) => self.errorMsg = error,
				() => {
					self.successMsg = " Create Successfully ";
					self.stateCreateModel.id= "";
					self.stateCreateModel.countryId= "";
					self.stateCreateModel.stateName= "";
				}
			);
			var newItem =self.stateCreateModel;
	...
	updateDeviation = function(index){
		var self = this;
		self.stateList[index].deviation = (self.stateList[index].stateName - self.stateList[index].stateName);
		self.http.editState( self.stateList[index] )
		.subscribe(
			(data) => {
...
deleteItem = function(indexdId){
			var self = this;
			this.gridOptions.api.setRowData(self.stateList);
			var id = self.stateList[indexdId].id
				this.http.deleteState(id)
				.subscribe(
				() => {

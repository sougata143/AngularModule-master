...
@Component({
  ...
  templateUrl: './indentApprovalListJute.component.html'
})
export class indentApprovalListJuteComponent implements OnInit {
		...
		this.http.getApprovalData(this.sessionData.sessionId)
			...
			() => {
				...
				for(var i = 0; i<self.approvalData.length; i++){
					if(self.approvalData[i].taskDesc == "Jute Indent"){

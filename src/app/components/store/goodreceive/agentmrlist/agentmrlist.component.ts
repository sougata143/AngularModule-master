...
@Component({
  ...
  templateUrl: './agentmrlist.component.html'
})


export class agentmrListComponent implements OnInit {
	...
	public columnDef : any = [
		{headerName: "MR Id", field: "materialGoodReceiveHeader.id", suppressMenu: true, minWidth : 350},
	...
	constructor(public http: HttpTestService, public session: sessionServices, public router: Router) { 
		var self = this;
		 ...
		 this.gridOptions.onRowClicked = function(params) {
			 if(params.data.materialGoodReceiveHeader.status != '4'){
				self.router.navigate(['jute/agentmrlist',params.data.materialGoodReceiveHeader.id]);
			 }
		 };
	}
			...
			.subscribe(
				...
				() => {
					for(var i=0; i<self.allOpenMrList.length; i++){
						if((self.allOpenMrList[i].materialGoodReceiveHeader.status == "3" || self.allOpenMrList[i].materialGoodReceiveHeader.status == "4") && self.allOpenMrList[i].materialGoodReceiveHeader.agentId != null && self.allOpenMrList[i].materialGoodReceiveHeader.warehouseNo == null){
